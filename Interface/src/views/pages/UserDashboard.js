...
const UserDashboard = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const compileResponse = await fetch("http://172.16.30.189/compile", {
        method: "POST",
        headers: {
      ...
      const saveResponse = await fetch("http://172.16.30.189/save_session", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
      ...
      try {
        ...
        const compiledAt = new Date().toISOString();
        const errorResponse = await fetch("http://172.16.30.189/save_session", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
